  ...
  return <section className="bg-[#fafafa] px-6 py-14 max-md:px-4 max-md:py-10" aria-labelledby="new-arrivals-title">
    <div className="mx-auto max-w-7xl">
      <div className="mb-7 flex items-center justify-between gap-4">
        ...
        <Link className="text-sm font-semibold text-[#195f3d] no-underline hover:underline" href="/collections/new-arrivals">View all</Link>
      </div>
...
function NewArrivalCard({ product }: { product: NewArrivalProduct }) {
  ...
  return <article className="relative flex min-w-0 flex-col border-r border-[#d5dadd] p-5 last:border-r-0 max-lg:nth-[2n]:border-r-0 max-md:border-r-0 max-md:border-b max-md:last:border-b-0">
    ...
    <Link className="group block no-underline" href={`/products/${product.handle}`}>
      ...
      <h3 className="mt-6 min-h-10 text-sm font-bold leading-relaxed text-[#263b4d] group-hover:text-[#195f3d] transition-colors">{product.title}</h3>
    </Link>
    <div className="mt-3 flex items-baseline gap-3">
      <span className="text-xl font-medium text-[#28714d]">{variants.length > 1 ? 'From ' : ''}{formatPrice(price.amount, price.currencyCode)}</span>
    ...
    <button
      className="mt-7 h-12 w-full cursor-pointer border-0 bg-[#195f3d] text-sm font-bold text-[#f5c400] transition-colors hover:bg-[#124b30] disabled:cursor-not-allowed disabled:opacity-60"
      type="button"
