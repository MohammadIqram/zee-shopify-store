...
export default function BestSellerProducts({ products }: { products: BestSellerProduct[] }) {
  return <section className="bg-[#F5F2E9] px-6 py-14 max-md:px-4 max-md:py-10" aria-labelledby="bestsellers-title">
    <div className="mx-auto max-w-7xl">
      <div className="mb-7 flex items-center justify-between gap-4">
        <h2 id="bestsellers-title" className="m-0 text-[24px] font-bold text-[#183D2B]">Bestsellers of the Week</h2>
        <Link className="text-sm font-semibold text-[#3F6B45] no-underline hover:text-[#183D2B] hover:underline" href="/collections/best-selling-products">View all</Link>
      </div>
      ...
      {products.length === 0 && <p className="border border-[#E7DFCF] p-10 text-center text-sm text-[#202722]">No bestselling products are available right now.</p>}
    </div>
  </section>;
}
...
function BestSellerCard({ product }: { product: BestSellerProduct }) {
  ...
  return <article className="relative flex min-w-0 flex-col border-r border-[#E7DFCF] p-5 last:border-r-0 max-lg:nth-[2n]:border-r-0 max-md:border-r-0 max-md:border-b max-md:last:border-b-0">
    ...
    <Link className="block no-underline" href={`/products/${product.handle}`}>
      ...
      <h3 className="mt-6 min-h-10 text-sm font-bold leading-relaxed text-[#202722] hover:text-[#183D2B] transition-colors">{product.title}</h3>
    </Link>
    ...
    <button className="mt-7 h-12 w-full cursor-pointer border-0 bg-[#183D2B] text-sm font-bold text-[#B59A5A] transition-colors hover:bg-[#3F6B45] disabled:cursor-not-allowed disabled:opacity-60" type="button" disabled={!variant?.availableForSale} onClick={() => {
      if (!variant) return;
